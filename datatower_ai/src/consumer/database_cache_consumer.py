...
#
# from datatower_ai.src.data.database.event_dao import DTEventEntity
#
...
#         self.__db_wm.execute(
#             _QueryFromDbTask(self.__db_wm, self.__network_wm, self.__http_service, self.__batch_size, self.__app_id)
#         )
#
...
#             _CounterMonitor["UploadFromDbTask-query"]))
#         Logger.log("[Statistics] 'mark_queried' time used: {:.2f}, avg: {:.2f}, count: {}".format(
#             TimeMonitor().get_sum("event_dao-set_queried"),
#             TimeMonitor().get_avg("event_dao-set_queried"),
#             _CounterMonitor["event_dao-set_queried"]))
...
#             _CounterMonitor["UploadFromDbTask-insert"]))
...
#             _CounterMonitor["UploadFromDbTask-delete"]))
...
#             _CounterMonitor["UploadFromDbTask-update_unquired"]))
...
#             _CounterMonitor["UploadFromDbTask-send"]))
...
#         ))
#         Logger.log("[Statistics] Http avg compress rate: {:.4f}".format(_CounterMonitor["http_avg_compress_rate"].value))
...
#     __sem = Semaphore()
#
#     def __init__(self, db_wm, nw_wm, http_service, batch_size, app_id):
#         self.__db_wm = db_wm
#         self.__nw_wm = nw_wm
...
#         _QueryFromDbTask.__sem.release()
#
#         dao = _DTDatabase().get_by_app_id(self.__app_id).event_dao
#
...
#             entities = _QueryFromDbTask.query_entities(dao, n, 0)
#
#             if len(entities) == 0:
...
#                 for split in _HttpService.approx_split_data_by_mb(etts, lambda x: x.data):
#                     if len(split) == 0:
#                         continue
#                     self.__nw_wm.execute(_UploadTask(self.__http_service, split, self.__after_upload))
#
#         self.__phase_end()
#
#     @staticmethod
...
#         else:
#             _CounterMonitor["UploadFromDbTask-fail"] += len(ids)
#
#         if success:
...
#             self.__db_wm.execute(
#                 _QueryFromDbTask(self.__db_wm, self.__nw_wm, self.__http_service, self.__batch_size, self.__app_id)
#             )
#
#
